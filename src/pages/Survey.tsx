...
const FALLBACK_SRC = 'https://ee.kobotoolbox.org/i/EfHgXxdw';
...
const Survey = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .maybeSingle()
      .then(({ data }) => {
        if (data?.value?.src) setSrc(data.value.src);
        if (data?.value?.height) setHeight(data.value.height);
      });
  }, []);
        ...
        <AnimatedSection>
          <div className="card bg-white overflow-hidden border border-slate-200">
            ...
            <iframe
              ...
              style={{ minHeight: `${parseInt(height, 10) || 600}px` }}
              allowFullScreen
            />
          </div>
        </AnimatedSection>
